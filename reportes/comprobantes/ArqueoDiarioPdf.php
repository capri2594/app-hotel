...
if ($_SERVER['REQUEST_METHOD'] !== 'POST' || empty($_POST['fecha_arqueo'])) die("Solicitud inválida.");
...
// Consultar pagos exactos de este día
...
$stmt->bind_param("s", $fecha_arqueo);
$stmt->execute();
...
<body>
    <div class="container">
        <table class="header" border="0" cellpadding="0" cellspacing="0">
            <tr>
                <td width="20%" class="text-center">' . ($logo1 ? '<img src="'.$logo1.'" height="60">' : '') . '</td>
                <td width="60%" class="company-details">
                    ' . ($logo2 ? '<img src="'.$logo2.'" height="35" style="margin-bottom: 5px;"><br>' : '') . '
                    <h1>' . htmlspecialchars(HOTEL_NOMBRE) . '</h1>
                    <p>NIT: ' . htmlspecialchars(HOTEL_NIT) . '</p>
                </td>
                <td width="20%" class="text-center">' . ($logo3 ? '<img src="'.$logo3.'" height="60">' : '') . '</td>
            </tr>
        </table>

        <div style="text-align: center; margin-bottom: 20px;">
            <h2 style="margin: 0; color: #680202;">ARQUEO DIARIO DE CAJA</h2>
            <p style="margin: 5px 0;"><strong>Fecha de Arqueo:</strong> ' . date('d/m/Y', strtotime($fecha_arqueo)) . ' | <strong>Emitido por:</strong> ' . htmlspecialchars($_SESSION['nombre_completo']) . '</p>
        </div>
...
if (count($lista_pagos) > 0) {
    foreach ($lista_pagos as $pago) {
        ...
        $html .= '<tr>
            <td>' . date('H:i', strtotime($pago['fecha'])) . '</td>
    ...
    $html .= '<tr><td colspan="5" class="text-center">No se registraron ingresos en esta fecha.</td></tr>';
}
...
$html .= '  </tbody>
        ...
        <table class="summary-table">
            ...
            <tr class="total-row"><th>INGRESO TOTAL:</th><td>' . htmlspecialchars(MONEDA_SIMBOLO) . ' ' . number_format($total_general, 2) . '</td></tr>
        </table>

        <table class="signatures" border="0" align="center" style="border-collapse: separate; border-spacing: 50px 0;">
            <tr>
                <td>Firma Recepcionista / Cajero</td>
                <td>Firma Administrador / Contabilidad</td>
            </tr>
        </table>
    </div>
</body>
...
$dompdf->render();
$dompdf->stream("arqueo-diario-" . $fecha_arqueo . ".pdf", array("Attachment" => false));